...
#define NODE_TABLE_CAP 64   // distinct nodes tracked at once; ~22 B/slot. Tunable per target.
#endif
...
public:
    ...
    // Identity verification state (set once after the self-sig check succeeds).
    bool verified(node_ref r) const;
    void mark_verified(node_ref r);
...
private:
    ...
    struct Slot { NodeId id; uint32_t last_ms; uint8_t flags; uint8_t gen; };
