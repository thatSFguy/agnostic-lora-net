...
static void test_replay_rejected() {
    uint8_t m[CTRL_MSG_BYTES];
    uint16_t n = ctrl_build(CTRL_POWER, 0xD97EEC3Au, 22, 100, SK, m, sizeof(m));
...
static void test_tamper_rejected() {
    uint8_t m[CTRL_MSG_BYTES];
    uint16_t n = ctrl_build(CTRL_POWER, 0xD97EEC3Au, 10, 7, SK, m, sizeof(m));
    m[6] = (uint8_t)22;                              // attacker edits power 10 -> 22
    ...
    n = ctrl_build(CTRL_POWER, 0xD97EEC3Au, 10, 7, SK, m, sizeof(m));
    m[7] ^= 0x01;
    TEST_ASSERT_EQUAL_UINT8(CTRL_BAD_SIG, ctrl_verify(m, n, PK, 0, nullptr));
}
...
static void test_wrong_key_rejected() {
    uint8_t m[CTRL_MSG_BYTES];
    uint16_t n = ctrl_build(CTRL_POWER, 0xD97EEC3Au, 10, 7, SK2, m, sizeof(m));  // attacker signs
